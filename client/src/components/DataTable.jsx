 ...
 * Props:
 *   columns: [{ key: 'field', label: 'Header', align: 'left'|'right'|'center', render: (val,row)=>node }]
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-y border-slate-200">
              {columns.map(col => (
                <th key={col.key}
                  className={`py-3 px-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider ${col.align === 'right' ? 'text-right' : col.align === 'center' ? 'text-center' : 'text-left'}`}>
                  {col.label}
                </th>
              ))}
            </tr>
